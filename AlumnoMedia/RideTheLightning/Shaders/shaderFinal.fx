...
float3 lightColor[3]; //Color RGB de las 3 luces
float4 lightPosition[3]; //Posicion de las 3 luces
float lightIntensity[3]; //Intensidad de las 3 luces
float lightAttenuation[3]; //Factor de atenuacion de las 3 luces
float4 spotLightDir[3]; //Direccion del cono de luz
float spotLightAngleCos[3]; //Angulo de apertura del cono de luz (en radianes)
float spotLightExponent[3]; //Exponente de atenuacion dentro del cono de luz

float4 direccionLinterna;//depende de la cámara
float4 posicionLinterna;//depende de la cámara
float3 colorLinterna;
float intensidadLinterna;
float atenuacionLinterna;
float angleCosLinterna; 
float exponentLinterna; 
...
LightingResult calcularLinterna(float3 Nn, float3 viewVector, float3 worldPosition)
{
	...
	float spotAtten = dot(-direccionLinterna.xyz, Ln);
		spotAtten = (spotAtten > angleCosLinterna )
						? pow(spotAtten, exponentLinterna)
						: 0.0;

	float intensity = intensidadLinterna * spotAtten / distAtten;

	//Ambient
	res.ambientLight = (dot(direccionLinterna.xyz, Nn) <= 0.0?1:0) * intensity * colorLinterna * materialAmbientColor;
	
	//Diffuse (N dot L)
	float3 n_dot_l = dot(Nn, Ln);
	res.diffuseLight = intensity * colorLinterna * materialDiffuseColor.rgb * max(0.0, n_dot_l);
	
	float3 n_dot_h = dot(Nn, Hn);
	res.specularLight = n_dot_l <= 0.0
			? float3(0.0, 0.0, 0.0)
			: (
				intensity * colorLinterna * materialSpecularColor 
				* pow(max( 0.0, n_dot_h), materialSpecularExp)
			);
...
float4 calcularColorFinal(LightingResult res0, LightingResult res1, LightingResult res2, LightingResult res3, float4 texelColor){
	float4 finalColor = float4(
		saturate(
			materialEmissiveColor + 
			(res0.ambientLight + res1.ambientLight + res2.ambientLight + res3.ambientLight) + 
			(res0.diffuseLight + res1.diffuseLight + res2.diffuseLight + res3.diffuseLight))
		* texelColor  + 
		(res0.specularLight + res1.specularLight + res2.specularLight + res3.specularLight) , 
